...
type View = "chat" | "settings" | "admin" | "login" | "register" | "2fa";
type MessageType = "text" | "image" | "voice" | "video" | "sticker";
...
// ─── Avatar Circle ────────────────────────────────────────────────────────────
function AvatarCircle({ text, size = 40, online }: { text:string; size?:number; online?:boolean }) {
...
export default function Index() {
  ...
  const [currentUser, setCurrentUser] = useState({ name:"Вы", avatar:"ВЫ", twoFa:false, isAdmin:false });
          ...
          <div className="bg-card rounded-2xl p-6 shadow-xl border border-border">
            ...
            {view === "login" ? (
              <div className="space-y-3">
                <input className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary transition"
                  placeholder="Номер телефона" value={loginForm.phone}
                  onChange={e => setLoginForm(p => ({ ...p, phone:e.target.value }))} />
                <input type="password" className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary transition"
                  placeholder="Пароль" value={loginForm.pass}
              ...
              <div className="space-y-3">
                <input className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary transition"
                  placeholder="Имя и фамилия" value={regForm.name}
                  onChange={e => setRegForm(p => ({ ...p, name:e.target.value }))} />
                <input className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary transition"
                  placeholder="Номер телефона" value={regForm.phone}
                  onChange={e => setRegForm(p => ({ ...p, phone:e.target.value }))} />
                <input type="password" className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary transition"
                  placeholder="Придумайте пароль" value={regForm.pass}
          ...
          <div className="bg-card rounded-2xl p-6 shadow-xl border border-border space-y-4">
            <div className="flex gap-2 justify-center">
              {Array.from({length:6}).map((_,i) => (
                <div key={i} className={`w-10 h-12 rounded-xl border-2 flex items-center justify-center text-xl font-bold transition-all ${twoFaCode[i] ? "border-primary bg-primary/10 text-primary" : "border-border"}`}>
                  {twoFaCode[i]||""}
                </div>
              ))}
            </div>
            <input className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm text-center outline-none focus:ring-2 focus:ring-primary tracking-widest font-mono"
              placeholder="Введите код" maxLength={6} value={twoFaCode}
              onChange={e => setTwoFaCode(e.target.value.replace(/\D/g,""))}
              onKeyDown={e => e.key==="Enter" && handleTwoFa()} />
            <button onClick={handleTwoFa} className="w-full bg-primary text-primary-foreground rounded-xl py-3 font-semibold hover:opacity-90 transition">
              Подтвердить
            </button>
            <button onClick={() => setView("login")} className="w-full text-muted-foreground text-sm hover:text-foreground transition">← Назад</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="h-screen flex overflow-hidden bg-background">
      ...
      `}>
        ...
        <div className="flex items-center gap-3 px-4 py-4 border-b border-border">
          <button onClick={() => { setView("settings"); setSettingsTab("profile"); }}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {currentUser.isAdmin && <span className="text-[10px] bg-primary/15 text-primary px-1.5 py-0.5 rounded-md font-medium flex-shrink-0">Админ</span>}
            </div>
        ...
        <div className="px-3 py-2.5">
          <div className="flex items-center gap-2 bg-muted rounded-xl px-3 py-2">
            ...
            <input className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground"
              placeholder="Поиск чатов..." value={searchQuery}
        ...
        <div className="flex-1 overflow-y-auto px-2 py-1 space-y-0.5">
          {filteredChats.map(chat => (
            <button key={chat.id}
              onClick={() => { setActiveChatId(chat.id); setMobileSidebar(false); setShowStickers(false); setShowAttach(false); }}
      ...
      {view==="chat" && activeChatId && activeChat ? (
        <main className="flex-1 flex flex-col min-w-0" onClick={() => { setShowEmojiReact(null); }}>
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card/60 glass">
            <button onClick={() => { setActiveChatId(null); setMobileSidebar(true); }}
                ...
                <div key={msg.id} className={`flex items-end gap-2 animate-slide-up ${isMine ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className="max-w-[72%] group relative">
                    ...
                    {showEmojiReact===msg.id && (
                      <div className={`absolute ${isMine ? "right-0" : "left-0"} -top-12 z-30 bg-card border border-border rounded-2xl px-2 py-1.5 flex gap-1 shadow-xl animate-scale-in`}
          ...
          <div className="px-3 py-3 bg-card/80 glass border-t border-border">
            <div className="flex items-end gap-2">
              <button onClick={() => { setShowAttach(p => !p); setShowStickers(false); }}
              ...
              <button onClick={() => { setShowStickers(p => !p); setShowAttach(false); }}
              ...
              <div className="space-y-3 animate-fade-in">
                <div className="bg-card rounded-2xl border border-border divide-y divide-border">
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <button onClick={() => { setCurrentUser(p => ({ ...p, twoFa:!p.twoFa })); notify(!currentUser.twoFa ? "2FA включена 🔐" : "2FA отключена"); }}
                  ...
                  <div key={user.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {user.frozen && <span className="text-[10px] bg-cyan-500/15 text-cyan-600 px-1.5 py-0.5 rounded-md font-medium">Заморожен</span>}
                      </div>
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      ...
                      <button onClick={() => { setAdminUsers(prev => prev.filter(u => u.id !== user.id)); notify(`${user.name} удалён`); }}
